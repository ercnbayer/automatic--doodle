...
func (jq *JobQuery) QueryJobappl() *JobApplicationQuery {
...
func (jq *JobQuery) Clone() *JobQuery {
	...
	return &JobQuery{
		...
		withJobappl: jq.withJobappl.Clone(),
...
func (jq *JobQuery) WithJobappl(opts ...func(*JobApplicationQuery)) *JobQuery {
	query := (&JobApplicationClient{config: jq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	jq.withJobappl = query
	return jq
}
...
func (jq *JobQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Job, error) {
	...
	if query := jq.withJobappl; query != nil {
		if err := jq.loadJobappl(ctx, query, nodes,
			func(n *Job) { n.Edges.Jobappl = []*JobApplication{} },
			func(n *Job, e *JobApplication) { n.Edges.Jobappl = append(n.Edges.Jobappl, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (jq *JobQuery) loadJobappl(ctx context.Context, query *JobApplicationQuery, nodes []*Job, init func(*Job), assign func(*Job, *JobApplication)) error {
	...
	query.Where(predicate.JobApplication(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(job.JobapplColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
