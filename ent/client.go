...
// QueryUsers queries the users edge of a RefreshToken.
func (c *RefreshTokenClient) QueryUsers(rt *RefreshToken) *UserQuery {
...
// QueryProfileImage queries the profile_image edge of a User.
func (c *UserClient) QueryProfileImage(u *User) *FileQuery {
...
// QueryCoverImage queries the cover_image edge of a User.
func (c *UserClient) QueryCoverImage(u *User) *FileQuery {
	query := (&FileClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		File, RefreshToken, User []ent.Hook
	}
	inters struct {
		File, RefreshToken, User []ent.Interceptor
	}
)
